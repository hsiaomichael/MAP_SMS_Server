...
class Handler(PCA_Parser.ContentHandler):	
  ...
  def startElement(self, name, attrs):
    try:
      ...
      if name == "version":			
        self.version =  attrs
      #elif name == "M3UA Message Class":	
      #  Msg = "DEBUG name=<%s>,attrs=<%s>" % (name,PCA_GenLib.HexDump(attrs))
      #  PCA_GenLib.WriteLog(Msg,0)	
  ...
  def characters(self,content):
    try:
      ...
      if content == "ASP Up (ASPUP)":
        self.SCTP_Message = self.M3UAMessage.getASUP_UP_Ack()
      elif content == "ASP Active (ASPAC)":
        self.SCTP_Message = self.M3UAMessage.getASP_Active_Ack()
  ...
  def getSCTPResponse(self):	
    try:
	Msg = "getSCTPResponse Init "
	PCA_GenLib.WriteLog(Msg,9)
        
			
	Msg = "getSCTPResponse OK"
	PCA_GenLib.WriteLog(Msg,9)
			
	return self.SCTP_Message
			
    except:
      Msg = "getSCTPResponse  error : <%s>,<%s> " % (sys.exc_type,sys.exc_value)
      PCA_GenLib.WriteLog(Msg,0)
      raise								
