...
class Writer:
  ...
  def getMessage(self,map_type,parameter_list):
	...
	YY = "%04d" % date_tuple[0]
      ...
      elif map_type == "SRI-SM-Ack":
        Msg = "construct SRI-SM-Ack"
        PCA_GenLib.WriteLog(Msg,1)
        MAP_Tag = chr(0xa2)
       
        noa = chr(0x91)
        
        nnn = PCA_GenLib.converStringToReverseBCD("886935000999")
        tag = chr(0x81)
        tag_data = noa + nnn
        ...
        #digits = PCA_GenLib.converStringToReverseBCD("4669767890123415")
        digits = chr(0x64)+chr(0x96)+chr(0x67) + chr(0x87) + chr(0x09) + chr(0x21) + chr(0x43) + chr(0xf5)
        tag = chr(0x04)
        tag_data = digits 
